        ...
        pair(
            ...
            preceded(
                ...
                space0_delimimited(ParseError::protect(
                    ...
                    crate::puppet_parser::term::parse_term,
                )),
            ),
        ),
    ...
    let mut parser = map(
        tuple((
            space0_delimimited(alt((
                ...
                crate::puppet_parser::expression::parse_string_expr,
                crate::puppet_parser::expression::parse_expression,
            ))),
        ...
        |(title, attributes)| {
            ...
            crate::puppet_lang::statement::Resource {
                attributes,
...
fn parse_resource_set(input: Span) -> IResult<crate::puppet_lang::statement::ResourceSet<Range>> {
    let parser = tuple((
        capture_comment,
        ...
        space0_delimimited(crate::puppet_parser::common::curly_brackets_delimimited(
            false,
            crate::puppet_parser::common::list_with_last_comment(terminated(
    ...
    map(
        ...
        |(comment, (virtual_tag, name), (_left_curly, list, right_curly))| {
            ...
            crate::puppet_lang::statement::ResourceSet {
...
fn parse_relation_type(input: Span) -> IResult<crate::puppet_lang::statement::RelationType<Range>> {
    alt((
        map(tag("->"), |tag: Span| {
            crate::puppet_lang::statement::RelationType {
                variant: crate::puppet_lang::statement::RelationVariant::ExecOrderRight,
                extra: (tag, tag).into(),
            }
        }),
        map(tag("~>"), |tag: Span| {
            crate::puppet_lang::statement::RelationType {
                variant: crate::puppet_lang::statement::RelationVariant::NotifyRight,
                extra: (tag, tag).into(),
            }
        }),
        map(tag("<-"), |tag: Span| {
            crate::puppet_lang::statement::RelationType {
                variant: crate::puppet_lang::statement::RelationVariant::ExecOrderLeft,
                extra: (tag, tag).into(),
            }
        }),
        map(tag("<~"), |tag: Span| {
            crate::puppet_lang::statement::RelationType {
                variant: crate::puppet_lang::statement::RelationVariant::NotifyLeft,
                extra: (tag, tag).into(),
            }
        }),
    ))(input)
...
fn parse_relation(input: Span) -> IResult<crate::puppet_lang::statement::RelationList<Range>> {
    let head_parser = alt((
        map(
            square_brackets_delimimited(
                false,
                crate::puppet_parser::common::comma_separated_list0_with_last_comment(alt((
                    map(
                        parse_resource_set,
                        crate::puppet_lang::statement::RelationEltVariant::ResourceSet,
                    ),
                    map(
                        crate::puppet_parser::resource_collection::parse_resource_collection,
                        crate::puppet_lang::statement::RelationEltVariant::ResourceCollection,
                    ),
                ))),
            ),
            |(left_curly, data, right_curly)| crate::puppet_lang::statement::RelationElt {
                data,
        ...
        map(
            ...
            |elt| crate::puppet_lang::statement::RelationElt {
                extra: Range::from((elt.extra(), elt.extra())),
                data: crate::puppet_lang::List {
                    value: vec![elt],
                    last_comment: vec![],
                },
            },
    ...
    let tail_parser = opt(map(
        ...
        |(comment, relation_type, relation_to)| crate::puppet_lang::statement::Relation {
            relation_type,
    ...
    map(pair(head_parser, tail_parser), |(head, tail)| {
        ...
        crate::puppet_lang::statement::RelationList {
...
fn parse_if_else(input: Span) -> IResult<StatementVariant<Range>> {
    ...
    let parser = map(parser, |(first, middle, else_block)| {
        ...
        let if_block = crate::puppet_lang::statement::ConditionAndStatement {
            condition,
            body: Box::new(body),
        ...
        let elsif_list: Vec<_> = middle
            ...
            .map(
                |(
                    ...
                    crate::puppet_lang::statement::ConditionAndStatement {
                        condition,
                        body: Box::new(body),
                        extra: (tag, right_curly).into(),
                        comment_before_body,
        ...
        crate::puppet_lang::statement::IfElse {
...
fn parse_unless(input: Span) -> IResult<StatementVariant<Range>> {
    ...
    map(
        ...
        |(op, condition, comment_before_body, (_left_curly, body, right_curly))| {
            StatementVariant::Unless(crate::puppet_lang::statement::ConditionAndStatement {
                condition,
                body: Box::new(body),
...
fn parse_case(input: Span) -> IResult<StatementVariant<Range>> {
    ...
    let parser_element = map(
        tuple((
            ...
            separated_list1(
                comma_separator,
                space0_delimimited(crate::puppet_parser::expression::parse_case_variant),
            ),
            tag(":"),
            space0_delimimited(parse_statement_block),
        )),
        |(comment, matches, _tag, (_left_curly, body, right_curly))| {
            crate::puppet_lang::statement::CaseElement {
    ...
    let parser = pair(
        ...
        curly_brackets_delimimited(
            true,
            crate::puppet_parser::common::list_with_last_comment(many0(parser_element)),
        ),
    );
    ...
    let parser = map(
        ...
        |((case_tag, condition), (_left_curly, elements, right_curly))| {
            crate::puppet_lang::statement::Case {
                condition,
                elements,
...
fn parse_resource_defaults(
    input: Span,
) -> IResult<crate::puppet_lang::statement::ResourceDefaults<Range>> {
    let kv_parser = pair(
    ...
    map(
        pair(
            crate::puppet_parser::identifier::camel_case_identifier,
